...
This article aims to replicate the case study developed by [Araujo et
al. (in review)](https://github.com/AraujoMat/treesliceR). We will also
present the workflow executed within the study for both a single
passerine phylogeny and multiple passerine phylogenies, in order to
control the results for phylogenetic uncertainty [Rangel et
al. (2015)](https://doi.org/10.1111/evo.12644).

Before starting our analysis, we’ll need to load the `ape`, `ggplot2`,
`ggpubr`, and `sf` packages. Additionally, if required, install the
...
Herein, we’ll compare the distribution of passerine species richness
...
used are subsets of the global bird phylogenies provided by [Jetz et
al. (2012)](https://doi.org/10.1038/nature11631).

First, we load the passerine phylogeny available within the package by
calling the `pass_trees` object:
...
We can prune the tips outside the defined thresholds from our phylogeny
using the
[`prune_tips()`](https://araujomat.github.io/treesliceR/reference/prune_tips.md)
function, setting the argument `qtl = TRUE` to prune the tips based on
quantile values:
...
The `method = 2` setting makes the function prune the tips older than
the specified temporal threshold. For more details, see the argument
descriptions by tipping
[`?prune_tips`](https://araujomat.github.io/treesliceR/reference/prune_tips.md).
...
![](Passeriformes-diversification_files/figure-html/unnamed-chunk-6-1.png)

Next, we need to obtain the species presence-absence matrix separately
for each phylogeny. Let’s start with the ancient phylogeny:

``` r
anc_mat <- pass_mat[, which(colnames(pass_mat) %in% ancient$tip.label)]
```

Now we’ll create another presence-absence matrix with the passerine
endemics, based on a quantile threshold of the 30th percentile of the
ancient species’ range size:

``` r
anc_end_mat <- anc_mat[which(colSums(anc_mat) <= quantile(colSums(anc_mat), 0.30))]
```

The same needs to be computed for recent species. We will subset their
presence-absence matrix for all recent species and another for their
endemics using the same criteria:
...
Finally, we’ll add all the species richness information to the grid
...
al. (in review) using the `ggplot2` package. Let’s start with the
ancient species:
...
![](Passeriformes-diversification_files/figure-html/unnamed-chunk-11-1.png)
...
![](Passeriformes-diversification_files/figure-html/unnamed-chunk-12-1.png)

Note that the final output may differ slightly from what was observed in
Araujo et al. (in review). However, it’s important to remember that here
we conducted the analysis using only a single passerine phylogeny. A
more comprehensive assessment using all phylogenies could be conducted
in subsequent sections below (for example in the section “*2. Passerines
study for multiple phylogenies*”).

### 1.2. Second framework example

Herein, we’ll calculate the CpB-rate of Australian passerines for both
*turnover* and *nestedness* components. Firstly, we’ll need to load the
assemblages containing the species matrix stored within the package by
calling the internal object `pass_mat`. Additionally, to calculate the
beta-diversity metrics, it is necessary to provide the adjacency matrix
containing the focal cells and their respective neighborhoods, which can
be accessed through the `AU_adj` object. Let’s examine the header of the
assemblage matrix containing the species, focusing on the first four
columns (or species):
...
We can run some sensitivity analysis to find the most parsimonious
number of slices to assess the CpB-rate patterns. But first, we need to
create a vector containing our desired number of slices for assessment:

``` r
vec <- c(250, 500, 750, 1000, 1250, 1500, 1750, 2000)
```

Let’s run the sensitivity analysis for both *turnover* and *nestedness*
components:
...
`CpR_sensitivity_plot` function. We use the `ggplot2` syntax to add a
vertical line showing our selected number of slices to run our
subsequent analysis:
...
![](OneTree_sensitivity.png)

Now, we can finally calculate the CpB-rates for *turnover* and
*nestedness* components, in this case, under a multisite approach
...
`AU_grid`. Let’s plot these patterns for the turnover component:
...
Herein, we’ll replicate the process from the previous section using
multiple (100) phylogenies. We’ll utilize all these phylogenies to
account for phylogenetic uncertainty associated with the inputted tips.

Let’s start by creating empty `data.frames` to store the species
richness for each category (i.e., ancient vs. recent, all species
vs. endemics):
...
Then, we can automate the procedures executed in the first section for
all our 100 phylogenies by simply using a `for()` loop to iterate over
each phylogeny:
...
Similarly as before, we can assign the mean species richness values of
each category to our `AU_grid` assemblages:
...
![](Passeriformes-diversification_files/figure-html/unnamed-chunk-25-1.png)

### 2.2. Second framework example

Once we have already decided on the number of slices in the previous
sections (in section “**1.2.**”), we’ll skip the sensitivity analysis
step to calculate the CpB-rates. Thus, here we’ll calculate the CpB-rate
for the 100 phylogenies using our previously defined criterion of 1000
slices.

First, we’ll run the CpB-rate analysis for the **turnover** component.
To obtain our CpB-rates faster for our 100 phylogenies, pay attention
that we set the
[`CpB()`](https://araujomat.github.io/treesliceR/reference/CpB.md)
function to run under parallel programming (using the argument `ncor`).
Before run it, you **must** confirm if the number of cores set (5) is
supported by your machine. Anyway, the following function can may take
some minutes to finish, but it can run faster if you are able to set a
...
Now, we capture can capture the mean of each parameter outputted in on
our list of
[`CpB()`](https://araujomat.github.io/treesliceR/reference/CpB.md)
outputs using the [`sapply()`](https://rdrr.io/r/base/lapply.html)
function. We’ll store them within a new data frame containing the format
that our
[`CpR_graph()`](https://araujomat.github.io/treesliceR/reference/CpR_graph.md)
is able to read (which is the same as the
[`CpB()`](https://araujomat.github.io/treesliceR/reference/CpB.md)
output):
...
Rangel, T. F., Colwell, R. K., Graves, G. R., Fučíková, K., Rahbek, C.
and Diniz-Filho, J. A. F. 2015. Phylogenetic uncertainty revisited:
Implications for ecological analyses. - Evolution 69(5): 1301–1312.
